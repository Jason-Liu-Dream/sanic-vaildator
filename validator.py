...
url: https://github.com/aleimu/flask-validator
"""
...
class Email(Validator):
    ...
    def __call__(self, value):
        try:
            ...
            if not (USER_REGEX.match(user_part) and DOMAIN_REGEX.match(
                    domain_part)):
                return False
            return True
        except:
...
class Not(Validator):
    ...
    def __init__(self, validator):
        ...
        self.err_message = getattr(validator, "not_message",
                                   "failed validation")
        self.not_message = getattr(validator, "err_message",
                                   "failed validation")
...
class InstanceOf(Validator):
    ...
    def __init__(self, base_class):
        ...
        class_names = base_class.__name__ if not isinstance(
            base_class, tuple) else ', '.join(
            [base.__name__ for base in base_class])
        self.err_message = "must be an instance of %s or its subclasses" % class_names
        self.not_message = "must not be an instance of %s or its subclasses" % class_names
...
class Length(Validator):
    ...
    def __init__(self, minimum, maximum=0):
        if not minimum and not maximum:
            raise ValueError(
                "Length must have a non-zero minimum or maximum parameter.")
        ...
        if minimum and maximum:
            self.err_message = self.err_messages["range"].format(
                ' ', minimum, maximum)
            self.not_message = self.err_messages["range"].format(
                ' not ', minimum, maximum)
        elif minimum:
...
# hook func
def validator_func(rules, strip=True, default=(False, None),
                   diy_func=None, release=False):
    """
    针对普通函数的参数校验的装饰器
    --- arbitrary argument lists(任意长参数)
    :param rules:参数的校验规则,map
    :param strip:对字段进行前后过滤空格
    :param default:将"" 装换成None
    :param diy_func:自定义的对某一参数的校验函数格式: {key:func},
           类似check, diy_func={"a": lambda x: x + "aa"})
    ...
    def decorator(f):
        ...
        def decorated_func(*args, **kwargs):
            # 入口保存原始的参数，如果release=True，则还原参数流回主程序
            args_bak = args[:]
            # fixme: 下面流程异常时,是否直接使用 原参数传入f
            ...
            try:
                ...
                print("validator_arbitrary_args catch err: ",
...
async def validator_sub(request, rules, strip=True,
                        default=(False, None),
                        diy_func=None, release=False):
    """返回dict,代替request.rel_url.query/request.json()使用,这个方法比较low ...
    :param aiohttp web request实例对象
    :param rules:参数的校验规则,map
    :param strip:对字段进行前后过滤空格
    :param default:将"" 装换成None
    :param diy_func:自定义的对某一参数的校验函数格式:
           {key:func},类似check, diy_func={"a": lambda x: x + "aa"})
    :param release:发生参数校验异常后是否依然让参数进入主流程函数
    """
    args_dict = OrderedDict()
    # 入口保存原始的参数，如果release=True，则还原参数流回主程序
    args_dict_copy = copy.deepcopy(args_dict)
    try:
        query_paramters = request.rel_url.query
        if query_paramters:
            args_dict.update(query_paramters)
        ...
        if json_parameters:
            args_dict.update(json_parameters)
        # strip
        if strip:
            do_strip(args_dict, modify=True)
        # default
...
async def limits(request, dict_args, strip, modify, default, diy_func, rules):
    # print('Verify parameter legality rules：%s' % dict_args)
    if dict_args.get("json", False):
        try:
            ...
            json_parameters = {}
        result, err = check(
            json_parameters, strip, modify,
            default, diy_func, rules)
        if not result:
            return result, err
    if dict_args.get("args", True) or dict_args.get("values", False):
        result, err = check(request.args, strip, modify, default,
                            diy_func,
                            rules)
        if not result:
            return result, err
    if dict_args.get("form", False) or dict_args.get("values", False):
        result, err = check(request.form, strip, modify, default,
                            diy_func,
                            rules)
        if not result:
            return result, err
...
def validator(rules, strip=True,
              modify=True,
              default=(False, None),
              diy_func=None, **dict_args):
    """修饰器版本的参数检查
    :param request:aiohttp web request实例对象
    :param rules:参数的校验规则,map
    ...
    :param diy_func:自定义的对某一参数的校验函数格式:
           {key:func},类似check, diy_func={"a": lambda x: x=="aa"})
    :param dict_args:检测范围,默认 json=False,args=Ture,form=False,
           values=False(values包括了args和form)
    """
    ...
    def decorator(f):
        @wraps(f)
        async def decorated_func(cls, request, *args, **kwargs):
            try:
                result, err = await limits(
                    request, dict_args, strip, modify,
                    default, diy_func, rules)
                if not result:
                    return sanic_json(
                        dict(error='parameter check get error',
                             detail=str(err)), status=400)
            except Exception as error:
                print("verify_args catch err: ", traceback.format_exc())
                return sanic_json(
                    dict(error='parameter check get exception',
                         detail=str(error)), status=500)
            return await f(cls, request, *args, **kwargs)
